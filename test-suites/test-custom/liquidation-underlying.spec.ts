...
makeSuite('LendingPool liquidation - liquidator receiving the underlying asset', (testEnv) => {
  ...
  it('Whitelist 100% test: Deposits AGT, borrows USDC', async () => {
    const { usdc, agt, users, pool, oracle } = testEnv;
  ...
  it('Whitelisted liquidator liquidates ~100% of debt when HF <= 0.95', async () => {
    ...
    await pool
      .connect(liquidator.signer)
      .liquidationCall(agt.address, usdc.address, borrower.address, fullDebt, false);
  ...
  it('Whitelist 50% test: Deposits AGT, borrows USDC', async () => {
    const { usdc, agt, users, pool, oracle } = testEnv;
